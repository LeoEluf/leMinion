...
#work
execute if score #items leMinions.temp < #max_storage leMinions.temp if score @s leMinions.timer matches 1.. run scoreboard players remove @s leMinions.timer 1
execute if score #items leMinions.temp < #max_storage leMinions.temp if score @s[tag=!leMinions.minion_setup] leMinions.timer matches ..0 run function le_minions:minions/work/start

#display storage full
execute if score #items leMinions.temp >= #max_storage leMinions.temp unless data entity @s data.leMinions.display{storage_full: 1b} run function le_minions:minions/display/storage_full
execute if score #items leMinions.temp < #max_storage leMinions.temp if data entity @s data.leMinions.display{storage_full: 1b} run function le_minions:minions/display/storage_not_full
